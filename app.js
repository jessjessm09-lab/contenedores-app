...
const SistemaContenedores = () => {
  ...
  const [pinGuardado, setPinGuardado] = useState('1234');
  ...
  const cargarDatos = () => {
    const datosGuardados = localStorage.getItem('registrosContenedores');
    if (datosGuardados) setRegistros(JSON.parse(datosGuardados));
    
    const choferesGuardados = localStorage.getItem('choferes');
    if (choferesGuardados) setChoferes(JSON.parse(choferesGuardados));
    
    const sectoresGuardados = localStorage.getItem('sectoresLugares');
    if (sectoresGuardados) setSectoresLugares(JSON.parse(sectoresGuardados));
    
    const pinGuardadoLS = localStorage.getItem('pinAdmin');
    if (pinGuardadoLS) setPinGuardado(pinGuardadoLS);
  };

  const guardarDatos = (nuevosRegistros) => {
    localStorage.setItem('registrosContenedores', JSON.stringify(nuevosRegistros));
    setRegistros(nuevosRegistros);
  };

  const guardarChoferes = (nuevosChoferes) => {
    localStorage.setItem('choferes', JSON.stringify(nuevosChoferes));
    setChoferes(nuevosChoferes);
  };

  const guardarSectores = (nuevosSectores) => {
    localStorage.setItem('sectoresLugares', JSON.stringify(nuevosSectores));
    setSectoresLugares(nuevosSectores);
  };

  const validarPinAdmin = () => {
    if (pinAdmin === pinGuardado) {
      setModo('admin');
      setPinAdmin('');
  ...
  const cambiarPin = () => {
    if (nuevoPinAdmin.length >= 4) {
      localStorage.setItem('pinAdmin', nuevoPinAdmin);
      setPinGuardado(nuevoPinAdmin);
      setNuevoPinAdmin('');
      mostrarMensaje('PIN actualizado correctamente', 'exito');
    } else {
      mostrarMensaje('El PIN debe tener al menos 4 dígitos', 'error');
    }
  };

  const agregarChofer = () => {
    if (nuevoChofer.trim()) {
      guardarChoferes([...choferes, nuevoChofer.trim().toUpperCase()]);
      setNuevoChofer('');
  ...
  const eliminarChofer = (chofer) => {
    if (confirm(`¿Está seguro de eliminar al chofer ${chofer}?`)) {
      guardarChoferes(choferes.filter(c => c !== chofer));
  ...
  const agregarSector = () => {
    if (nuevoSector.trim()) {
      guardarSectores({ ...sectoresLugares, [nuevoSector.trim().toUpperCase()]: [] });
      setNuevoSector('');
  ...
  const eliminarSector = (sector) => {
    if (confirm(`¿Está seguro de eliminar el sector ${sector}?`)) {
      const nuevosSectores = { ...sectoresLugares };
      delete nuevosSectores[sector];
      guardarSectores(nuevosSectores);
  ...
  const agregarLugar = () => {
    if (sectorParaLugar && nuevoLugar.trim()) {
      ...
      guardarSectores(nuevosSectores);
      setNuevoLugar('');
      mostrarMensaje('Lugar agregado correctamente', 'exito');
    }
  };

  const eliminarLugar = (sector, lugar) => {
    if (confirm(`¿Está seguro de eliminar el lugar ${lugar}?`)) {
      const nuevosSectores = { ...sectoresLugares };
      nuevosSectores[sector] = nuevosSectores[sector].filter(l => l !== lugar);
      guardarSectores(nuevosSectores);
      mostrarMensaje('Lugar eliminado', 'exito');
    }
  };

  const registrarEntrega = () => {
    if (!choferActual || !sectorSeleccionado || !lugarSeleccionado || !numContenedor) {
    ...
    const nuevoRegistro = {
      ...
      fechaEntregaFormato: fechaEntrega.toLocaleString('es-AR'),
    ...
    guardarDatos([...registros, nuevoRegistro]);
    setNumContenedor('');
  ...
  const registrarRetiro = () => {
    if (!choferActual || !sectorSeleccionado || !lugarSeleccionado || !numContenedor || !kilos) {
      mostrarMensaje('Por favor complete todos los campos incluyendo los kilos', 'error');
      return;
    }

    const entregaIndex = registros.findIndex(
    ...
    if (entregaIndex === -1) {
      mostrarMensaje('No se encontró una entrega pendiente para este contenedor en este lugar', 'error');
      return;
    }
    ...
    const fechaEntrega = new Date(nuevosRegistros[entregaIndex].fechaEntrega);
    const dias = Math.floor((fechaRetiro - fechaEntrega) / (1000 * 60 * 60 * 24));

    nuevosRegistros[entregaIndex] = {
      ...nuevosRegistros[entregaIndex],
      estado: 'COMPLETADO',
      choferRetiro: choferActual,
      fechaRetiro: fechaRetiro.toISOString(),
      fechaRetiroFormato: fechaRetiro.toLocaleString('es-AR'),
      kilos: parseFloat(kilos),
      dias: dias
    };

    guardarDatos(nuevosRegistros);
    setNumContenedor('');
    setKilos('');
  ...
  const eliminarRegistro = (id) => {
    if (confirm('¿Está seguro de eliminar este registro?')) {
      guardarDatos(registros.filter(r => r.id !== id));
    ...
    if (filtroMes) {
      datosExportar = registrosCompletos.filter(r => {
        const fechaRegistro = new Date(r.fechaEntrega);
          ...
          <table>
            <thead>
              <tr>
                ...
                <th colspan="5">ENTREGA</th>
                <th colspan="5">RETIRO</th>
                <th colspan="1">KGS.</th>
    ...
    datosExportar.forEach(r => {
      ...
      const fechaRetiro = new Date(r.fechaRetiro);
      htmlContent += `
        <tr>
          <td>${r.sector}</td>
          <td>${r.lugar}</td>
    ...
    htmlContent += `
            ...
            <tfoot>
              <tr class="total">
                <td colspan="11" style="text-align: right;">TOTAL VIAJES</td>
                <td class="numero">${totalViajesDobles}</td>
                <td colspan="2" style="text-align: center;">TOTAL KG.</td>
              </tr>
              <tr class="total">
                <td colspan="11"></td>
                <td></td>
                <td class="numero" colspan="2">${totalKilos.toLocaleString('es-AR')}</td>
              </tr>
    `;
  ...
  const lugaresPorSector = sectorSeleccionado ? sectoresLugares[sectorSeleccionado] : [];// ===== RENDER JSX =====
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-4">
      <div className="max-w-6xl mx-auto">
            ...
            <div className="max-w-md mx-auto mt-8">
              <div className="flex gap-4 mb-6">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-6 py-4 rounded-lg font-semibold bg-blue-600 text-white hover:bg-blue-700 transition-all"
                >
                  <Truck size={24} />
                  Modo Chofer
                </button>
                <button
                  className="flex-1 flex items-center justify-center gap-2 px-6 py-4 rounded-lg font-semibold bg-green-600 text-white hover:bg-green-700 transition-all"
                >
                  <UserCog size={24} />
                  Administrador
                </button>
              ...
              <div className="bg-gray-50 p-6 rounded-lg">
                ...
                <input
                  ...
                  onKeyPress={(e) => e.key === 'Enter' && validarPinAdmin()}
          ...
          {modo !== 'login' && (
            <div className="flex gap-4">
              ...
              <button
                ...
                className="ml-auto px-6 py-3 rounded-lg font-semibold bg-gray-600 text-white hover:bg-gray-700 transition-all"
              >
                Salir
              </button>
            </div>
        ...
        {modo === 'chofer' && (
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-lg p-6">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">Datos del Viaje</h2>
                    ...
                    <div key={r.id} className="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded-lg">
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">Contenedor #{r.contenedor}</p>
                          <p className="text-xs text-gray-500">Entregado: {r.fechaEntregaFormato} por {r.chofer}</p>
                        </div>
              ...
              {mostrarConfig && (
                <div className="mb-6 bg-gray-50 p-6 rounded-lg space-y-6">
                  ...
                  <div className="border-b pb-4">
                    <h4 className="font-bold text-gray-700 mb-3">Cambiar PIN de Administrador</h4>
                    <div className="flex gap-2">
                      <input
                        type="password"
                        value={nuevoPinAdmin}
                  ...
                  {/* Gestión de Sectores */}
                  <div className="border-b pb-4">
                    <h4 className="font-bold text-gray-700 mb-3">Gestión de Sectores</h4>
                    <div className="flex gap-2 mb-3">
                      <input
                        type="text"
                        value={nuevoSector}
                      ...
                      {Object.keys(sectoresLugares).map((sector, idx) => (
                        <div key={idx} className="bg-white p-3 rounded-lg">
                          ...
                          <div className="ml-4 text-sm text-gray-600">
                            {sectoresLugares[sector].length} lugares configurados
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {/* Gestión de Lugares */}
                  <div>
                    <h4 className="font-bold text-gray-700 mb-3">Gestión de Lugares por Sector</h4>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-100">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left font-semibold text-gray-700">Lugar</th>
                        ...
                        .map(r => (
                          <tr key={r.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="px-3 py-3 text-xs text-gray-600">
                              <div>{r.chofer}</div>
                              <div className="text-gray-500">{new Date(r.fechaEntrega).toLocaleDateString('es-AR')}</div>
                            </td>
                            <td className="px-3 py-3 text-xs text-gray-600">
                              {r.fechaRetiro ? (
                                <>
                                  <div>{r.choferRetiro}</div>
                                  <div className="text-gray-500">{new Date(r.fechaRetiro).toLocaleDateString('es-AR')}</div>
                                </>
                              ) : (
        ...
        <div className="mt-6 text-center text-sm text-gray-600">
          <p>✓ Funcionamiento offline - Los datos se guardan localmente en el dispositivo</p>
        </div>
      </div>
    </div>
  );
