...
const SistemaContenedores = () => {
  ...
  const cargarDatos = async () => {
    ...
    // Si es chofer, solo ver sus registros
  ...
  const registrarRetiro = async () => {
    if (!choferActual || !sectorSeleccionado || !lugarSeleccionado || !numContenedor || !kilos || !internoSeleccionado) {
      mostrarMensaje('Por favor complete todos los campos incluyendo los kilos', 'error');
      return;
    }

    // Buscar la entrega pendiente
    const entrega = registros.find(
      r => r.contenedor === numContenedor && 
           r.sector === sectorSeleccionado && 
           r.lugar === lugarSeleccionado && 
           r.interno === internoSeleccionado &&
           r.estado === 'PENDIENTE_RETIRO'
    );

    if (!entrega) {
      mostrarMensaje('No se encontró una entrega pendiente para este contenedor e interno en este lugar', 'error');
      return;
    }
        ...
        {modo === 'chofer' && (
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="space-y-4">
                {/* Chofer (solo lectura para choferes, editable para admin) */}
                {userProfile?.rol === 'admin' ? (
                  <div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-100">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-center font-semibold text-gray-700">Int.</th>
